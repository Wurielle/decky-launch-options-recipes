...
This repository packages launch option recipes for the Decky Launch Options plugin. Recipe sources live in `recipes/*.ts`; each file exports one `Recipe` object and `recipes/types.ts` defines the shared shape. `recipes.json` is generated from those sources and should not be hand-edited except to inspect output. The Decky frontend is in `src/`, backend helper code is in `backend/`, static assets are in `assets/`, default data is in `defaults/`, and recipe build tooling is in `scripts/`.
...
Use TypeScript ESM style and match nearby files. Recipe files use hyphen-case names, for example `lossless-scaling.ts`, and import the shared type with `import type { Recipe } from './types.js'`. Prefer 4-space indentation in recipe objects, trailing commas, and explicit `satisfies Recipe`.
