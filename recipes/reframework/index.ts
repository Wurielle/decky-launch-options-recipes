...
const runScript = (scriptName: string) => `bash -c '${hostRuntime} curl -fsSL --retry 3 --retry-delay 1 "${scriptsBaseUrl}/${scriptName}" | ${hostRuntime} bash -s -- "$STEAM_COMPAT_INSTALL_PATH"; exec "$@"' -- %command%`
